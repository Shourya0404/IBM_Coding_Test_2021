...
app.post('/cart', function (req, res) {
  ...
  //db.run(`SELECT EXISTS (SELECT * from card_item where product_id=${req.query.productId}`);
  //db.run(`SELECT EXISTS (SELECT * from card_item where product_id=${req.query.productId})`,(err) => {
  //  if (err) {
  //    res.status(400).json({ "error": err.message });
  //    return;
  //  
